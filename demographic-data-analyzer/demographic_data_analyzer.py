...
def calculate_demographic_data(print_data=True):
    ...
    min_work_hours = df['hours-per-week'].sort_values().values[0]
    ...
    tmp = (df[df['salary'] == '>50K']['native-country'].value_counts() /
           df['native-country'].value_counts()).sort_values(ascending=False)
    highest_earning_country = tmp.index[0]
    highest_earning_country_percentage = round(tmp.values[0] * 100, 1)
